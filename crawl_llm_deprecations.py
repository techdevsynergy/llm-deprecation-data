...
MODEL_REGEX_BY_PROVIDER = {
    "openai": re.compile(
        r"\b(?:gpt|chatgpt|o[0-9]|ft-(?:gpt|o[0-9])|computer-use|"
...
def is_probable_model_id(provider: str, token: str) -> bool:
    ...
    if provider == "openai":
        return bool(
            re.match(
                r"^(gpt|chatgpt|o[0-9]|ft-(?:gpt|o[0-9])|computer-use|"
...
def extract_replacement(text: str, provider: str) -> Optional[str]:
    if provider == "gemini":
        ...
        for part in re.split(r"\s+or\s+", text, flags=re.IGNORECASE):
            match = re.search(
                r"\b(?P<family>gemini|gemma)\s+(?P<version>\d+(?:\.\d+)*)"
                r"(?P<variant>(?:\s+[a-z]+(?:-[a-z]+)*)*)\s*$",
                part.strip(),
                re.IGNORECASE,
            )
            if match:
                marketing_ids.append(
                    "-".join(
                        (
                            match.group("family"),
                            match.group("version"),
                            match.group("variant").strip().replace(" ", "-"),
                        )
                    ).rstrip("-").lower()
                )
        if marketing_ids:
            return " or ".join(marketing_ids)
...
def parse_tables(
    ...
    for table in tables:
        ...
        lifecycle_date_header = any(
            value in header_str
            for value in ("retirement date", "discontinuation date", "shutdown date", "sunset")
        )
        lifecycle_model_header = any(
            value in header_str
            for value in ("model id", "model snapshot", "model / system", "deprecated model", "legacy model")
        )
        if lifecycle_date_header and lifecycle_model_header:
            model_idx = next(
                (
                    i
            ...
            repl_idx = next(
                (
                    i
                    for i, h in enumerate(header)
                    if "recommended upgrade" in h or "replacement" in h or "substitute" in h or "successor" in h
                ),
                None,
            )
            for row in table[1:]:
                ...
                retirement_lower = retirement.lower()
                if retirement.strip() and not sunset and not any(
                    marker in retirement_lower
                    for marker in (
                        "n/a",
                        "no retirement date announced",
                        "not before",
                    ...
                    parse_warnings.append(
                        f"{provider}:{','.join(model_ids)} lifecycle row has unparsed date "
                        f"{retirement!r} ({url})"
                    )
                replacement = row[repl_idx] if repl_idx is not None and repl_idx < len(row) else ""
                ...
                for model_id in model_ids:
                    status = choose_status("deprecated retirement", sunset) if sunset else "active"
                    merge_candidate(
                        store=out,
                        provider=provider,
                        model_id=model_id,
    ...
    for match in explicit.finditer(text_blob):
        ...
        if replacement is None:
            nearby_model_ids = extract_model_ids(sentence, provider)
            replacement = next((candidate for candidate in nearby_model_ids if candidate != model_id), None)
        status = "retired" if sunset < date.today().strftime("%Y-%m-%d") else "deprecated"
        merge_candidate(
            store=out,
            provider=provider,
